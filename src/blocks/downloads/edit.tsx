...
const Edit: React.FC<EditProps> = ({ isSelected, attributes }) => {
  ...
  blockProps.className = `${blockProps.className.replace(
    ...
    ""
  )}${!downloads || downloads.length === 0 ? " is-empty" : ""}`;
  ...
  const onSelectNewFile = (file) => {
    ...
    if (!newDownloads.some((item) => item.id === file.id)) {
      setDownloads([
        ...newDownloads,
        { id: file.id, title: file.title ? file.title : file.filename },
      ]);
    }
  };
        ...
        >
          {!downloads || downloads.length === 0 ? (
            ...
            <table className="sg-download__list color-primary f-s txt-ctr">
              {downloads &&
                downloads.map((download, i: number) => (
                  <tr key={i} className="f-s">
                    <td className="sg-icon-file-text">
                      {isSelected ? (
                        <RichText
                          tagName="span"
                    ...
                    {isSelected && (
                      <>
                        <td>
                          <MediaUploadCheck>
                            <MediaUpload
                              onSelect={(file: File) => onChangeFile(file, i)}
                              allowedTypes={ALLOWED_MEDIA_TYPES}
                              value={download.id}
                              render={({ open }) => (
                                <Button
